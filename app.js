...
const DEFAULT_THEMES = [
  { name: 'Azul', color: '#1478ee' },
  { name: 'Vermelho', color: '#e53935' },
  { name: 'Verde', color: '#15915f' },
  { name: 'Roxo', color: '#6c5ce7' },
  { name: 'Laranja', color: '#ed7d21' },
  { name: 'Rosa', color: '#d9467a' },
  { name: 'Turquesa', color: '#0891a2' },
  { name: 'Amarelo', color: '#c58b00' },
  { name: 'Índigo', color: '#3f51b5' },
  { name: 'Cinza', color: '#5f6673' }
];
...
const DEFAULT_SETTINGS = {
  ...
  activeTheme: '#1478ee',
  darkMode: false,
  costs: { 'T-shirt': 15, 'Camisola': 25, 'Kit': 45, custom: 5, patch: 0, meias: 0, special: 0 },
  prices: { 'T-shirt': 15, 'Camisola': 25, 'Kit': 45, custom: 5, patch: 0, meias: 0, special: 0 }
};
...
function loadSettings() {
  try {
    ...
    s.themes = (s.themes || DEFAULT_THEMES).map(theme => typeof theme === 'string'
      ? { name: theme, color: theme }
      : theme);
    s.activeTheme = s.activeTheme || DEFAULT_SETTINGS.activeTheme;
...
function orderCost(order) {
  ...
  const patchCount = Array.isArray(order.patches) && order.patches.length
    ? order.patches.filter(Boolean).length
    : (order.patchado && order.patchNome ? 1 : 0);
  const patchCost = patchCount * (settings.costs.patch || 0);
  const socksCost = order.meias && order.meias.length ? (settings.costs.meias || 0) : 0;
  return cost + equipmentCost + customCost + patchCost + socksCost;
}
...
function matchesSearch(order, term) {
  ...
  const patchNames = Array.isArray(order.patches) ? order.patches : [order.patchNome || ''];
  const socks = Array.isArray(order.meias) ? order.meias : [];
  return (order.nome || '').toLowerCase().includes(t)
    || (order.clube || '').toLowerCase().includes(t)
    || (order.equipamento || '').toLowerCase().includes(t)
    || patchNames.some(patch => patch.toLowerCase().includes(t))
    || socks.some(pair => pair.toLowerCase().includes(t))
...
function orderCardHTML(order) {
  ...
  const patchNames = Array.isArray(order.patches) ? order.patches : [order.patchNome || ''];
  const socks = Array.isArray(order.meias) ? order.meias : [];
  const patchLine = order.patchado && patchNames.some(Boolean)
    ? ` · Patch: ${patchNames.filter(Boolean).map(escapeHTML).join(', ')}`
    : '';
  const socksLine = order.meias && socks.length
    ? ` · Meias: ${socks.filter(Boolean).map(escapeHTML).join(', ')}`
    : '';
    ...
    <div class="order-card" data-id="${order.id}">
      <div class="order-card-top">
        <div>
          ...
          <div class="order-card-sub">${escapeHTML(order.tipo)} · ${escapeHTML(order.clube)}${equipamentoLine}${tamanhoLine}${customLine}${patchLine}${socksLine}</div>
        </div>
...
function renderSettings() {
  ...
  document.getElementById('price-meias').value = settings.costs.meias;
  ...
  document.getElementById('sale-price-meias').value = settings.prices.meias;
  ...
  themesList.innerHTML = settings.themes.map(theme => `
    <li class="theme-option${theme.color === settings.activeTheme ? ' is-active' : ''}" data-theme="${escapeHTML(theme.color)}">
      <span class="theme-swatch" style="background:${escapeHTML(theme.color)}"></span>
...
['price-tshirt', 'price-camisola', 'price-kit', 'price-custom', 'price-patch', 'price-meias', 'price-special'].forEach(id => {
  document.getElementById(id).addEventListener('change', () => {
    ...
    settings.costs.meias = Number(document.getElementById('price-meias').value) || 0;
...
['sale-price-tshirt', 'sale-price-camisola', 'sale-price-kit', 'sale-price-custom', 'sale-price-patch', 'sale-price-meias', 'sale-price-special'].forEach(id => {
  document.getElementById(id).addEventListener('change', () => {
    ...
    settings.prices.meias = Number(document.getElementById('sale-price-meias').value) || 0;
...
const addPatchBtn = document.getElementById('btn-add-patch');
const socksCheckbox = document.getElementById('f-meias');
const socksFields = document.getElementById('meias-fields');
const socksList = document.getElementById('meias-list');
const addSocksBtn = document.getElementById('btn-add-meias');
...
function addPatchField(value = '') {
  const row = document.createElement('div');
  row.className = 'patch-row';
  row.innerHTML = `
    <input type="text" class="patch-input" placeholder="Ex: Liga dos Campeões">
    <button class="patch-remove" type="button" aria-label="Remover patch">&times;</button>
  `;
  row.querySelector('.patch-input').value = value;
...
function setPatchFields(patches) {
  patchList.innerHTML = '';
  patches.forEach(addPatchField);
}

function getPatchNames() {
  return [...patchList.querySelectorAll('.patch-input')]
    .map(input => input.value.trim())
    .filter(Boolean);
}

function getPatchCount() {
  return patchCheckbox.checked ? patchList.children.length : 0;
}

function addSocksField(value = '') {
  const row = document.createElement('div');
  row.className = 'patch-row';
  row.innerHTML = `
    <input type="text" class="patch-input" placeholder="Ex: Meias Porto">
    <button class="patch-remove" type="button" aria-label="Remover meias">&times;</button>
  `;
  row.querySelector('.patch-input').value = value;
  row.querySelector('.patch-remove').addEventListener('click', () => row.remove());
  socksList.appendChild(row);
}

function setSocksFields(socks) {
  socksList.innerHTML = '';
  socks.forEach(addSocksField);
}

function getSocks() {
  return [...socksList.querySelectorAll('.patch-input')]
    .map(input => input.value.trim())
    .filter(Boolean);
}
...
function openModal(order = null) {
  ...
  if (order) {
    ...
    setPatchFields(Array.isArray(order.patches) && order.patches.length
      ? order.patches
      : [order.patchNome || '']);
    socksCheckbox.checked = Array.isArray(order.meias) && order.meias.length > 0;
    setSocksFields(socksCheckbox.checked ? order.meias : ['']);
    ...
    socksFields.hidden = !socksCheckbox.checked;
...
patchCheckbox.addEventListener('change', () => {
  ...
  if (patchCheckbox.checked && !patchList.children.length) setPatchFields(['']);
  applyAutoPrice();
});

addPatchBtn.addEventListener('click', () => addPatchField());

socksCheckbox.addEventListener('change', () => {
  socksFields.hidden = !socksCheckbox.checked;
  if (socksCheckbox.checked && !socksList.children.length) setSocksFields(['']);
});

addSocksBtn.addEventListener('click', () => addSocksField());
...
function applyAutoPrice() {
  ...
  const patchPrice = getPatchCount() * (settings.prices.patch || 0);
  const socksPrice = socksCheckbox.checked ? (settings.prices.meias || 0) : 0;
  precoInput.value = base + equipmentPrice + extra + patchPrice + socksPrice;
}
...
orderForm.addEventListener('submit', (e) => {
  ...
  const hasPatch = patchCheckbox.checked && patchList.children.length > 0;
  ...
  const orderData = {
    ...
    patchado: hasPatch,
    patches: hasPatch ? patchNames : [],
    patchNome: hasPatch ? patchNames[0] : '',
    meias: socksCheckbox.checked ? socks : [],
    preco: Number(precoInput.value) || 0,
    custo: id
      ? (orders.find(o => o.id === id).custo ?? ((settings.costs[tipoSelect.value] || 0) + (isSpecialEquipment(document.getElementById('f-equipamento').value) ? (settings.costs.special || 0) : 0) + (isCustom ? (settings.costs.custom || 0) : 0) + (getPatchCount() * (settings.costs.patch || 0)) + (socks.length ? (settings.costs.meias || 0) : 0)))
      : (settings.costs[tipoSelect.value] || 0) + (isSpecialEquipment(document.getElementById('f-equipamento').value) ? (settings.costs.special || 0) : 0) + (isCustom ? (settings.costs.custom || 0) : 0) + (getPatchCount() * (settings.costs.patch || 0)) + (socks.length ? (settings.costs.meias || 0) : 0),
...
function applyThemePalette() {
  const activeTheme = settings.activeTheme || DEFAULT_SETTINGS.activeTheme;
  document.documentElement.style.setProperty('--accent', activeTheme);
  document.documentElement.style.setProperty('--accent-dark', activeTheme);
  document.documentElement.style.setProperty('--accent-light', activeTheme);
  document.documentElement.style.setProperty('--brand-gradient', `linear-gradient(135deg, ${activeTheme}, color-mix(in srgb, ${activeTheme} 65%, #2d2d3a))`);
...
document.getElementById('themes-list')?.addEventListener('click', (e) => {
  ...
  if (!li) return;
  settings.activeTheme = li.dataset.theme;
  saveSettings(settings);
  applyThemePalette();
  renderSettings();
});
